...
internal class PwaAssetGenerator : IPwaAssetGenerator
{
    ...
    public string ServiceWorker()
    {
        ...
        // Lowercased here rather than in the worker: the prefixes are known at generation time,
        // so normalising them once is cheaper than normalising them on every fetch.
        var apiPrefix = J(sw.ApiPrefix.ToLowerInvariant());
        var skip = JsonSerializer.Serialize(sw.SkipPaths.Select(p => p.ToLowerInvariant()).ToArray());
    ...
    fetch(NAV_FALLBACK)
      .then(async (resp) => {
        ...
        if (resp.redirected) {
          ...
          repaired = new Response(await resp.arrayBuffer(), {
            status: resp.status,
            statusText: resp.statusText,
            headers,
          });
        }
        return caches.open(SHELL).then((c) => c.put(NAV_FALLBACK, repaired));
      })
...
function storable(resp) {
  ...
  return cacheable(resp) && !resp.redirected;
}
...
// ASP.NET routing matches paths case-insensitively and decodes percent-escapes before it routes.
// URL.pathname does neither, so /Umbraco/, /UMBRACO/ and /%75mbraco/ all reach the backoffice while
// a raw startsWith against "/umbraco/" misses every one of them. Every path comparison in this file
// goes through here, so a new one cannot reintroduce the gap by forgetting to normalise.
function normalisePath(pathname) {
  let p = pathname;
  try {
    p = decodeURIComponent(p);
  } catch (e) {
    // A malformed escape such as /%zz is not decodable. Comparing the raw form is the safe
    // fallback: it can only fail to match, and an unmatched path is simply left uncached.
  }
...
""";
    ...
    public string Client(string pathBase)
    {
        ...
        var hideOn = JsonSerializer.Serialize(prompt.HideOnPaths.Select(p => p.ToLowerInvariant()).ToArray());
...
(function () {
  ...
  // Same normalisation as the service worker, and for the same reason: /Umbraco/ and /%75mbraco/
  // both serve the backoffice, so a raw pathname comparison shows the install prompt there.
  function normalisePath(pathname) {
    var p = pathname;
    try {
      p = decodeURIComponent(p);
    } catch (e) {
      // Malformed escape. The raw form can only fail to match, which just shows the prompt.
    }
    return p.toLowerCase();
  }
